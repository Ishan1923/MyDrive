...
def manage_files(request):
    ...
    unique_files = {}
    files_with_versions_list = []
    
    for f in all_files:
        if f.file.name not in unique_files:
            unique_files[f.file.name] = True
            
            # Fetch versions from AWS S3
            versions = get_s3_file_versions(f.file.name)
            
            # Construct dictionary matching template syntax (item.obj and item.versions)
            files_with_versions_list.append({
                'obj': f,
                'versions': versions
            })
    ...
    return render(request, 'file_manager.html', {
        'form': form, 
        'files_with_versions': files_with_versions_list
    })